...
$namespace = '\crud_booster_voila\crudbooster\controllers';

Route::group(['middleware' => ['api', '\crud_booster_voila\crudbooster\middlewares\CBAuthAPI'], 'namespace' => 'App\Http\Controllers'], function () {
...
Route::group([
    'middleware' => ['web', '\crud_booster_voila\crudbooster\middlewares\CBBackend'],
    ...
    if (Request::is(config('crudbooster.ADMIN_PATH'))) {
        ...
        if ($menus) {
            if ($menus->type == 'Statistic') {
                Route::get('/', '\crud_booster_voila\crudbooster\controllers\StatisticBuilderController@getDashboard');
...
Route::group([
    ...
    if (Request::is(config('crudbooster.ADMIN_PATH'))) {
        $menus = DB::table('cms_menus')->where('is_dashboard', 1)->first();
        if (! $menus) {
            CRUDBooster::routeController('/', 'AdminController', $namespace = '\crud_booster_voila\crudbooster\controllers');
        }
    }

    CRUDBooster::routeController('api_generator', 'ApiCustomController', $namespace = '\crud_booster_voila\crudbooster\controllers');
    ...
    try {
        ...
        foreach ($moduls as $v) {
            if (@$v->path && @$v->controller) {
                CRUDBooster::routeController($v->path, $v->controller, $namespace = '\crud_booster_voila\crudbooster\controllers');
            }
        }
    } catch (Exception $e) {
...
Route::get('admin/getModulesItem/{val}','crud_booster_voila\crudbooster\controllers\MenusClientController@getModulesItem');
