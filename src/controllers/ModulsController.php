<?php namespace crud_booster_voila\crudbooster\controllers;
...
use crud_booster_voila\crudbooster\fonts\Fontawesome;
...
class ModulsController extends CBController
{
    ...
    public function getAdd()
    {
        ...
        return redirect()->route("ModulsConttrollerGetStep1");
    }
    ...
    public function postStep2()
    {
        ...
        $has_page_client = Request::get('has_page_client');
        $main_field = Request::get('main_field');
        $lang_effected = Request::get('lang_effected');
        ...
        if (! Request::get('id')) {
            ...
            //updated ny fesal
            DB::table($this->table)->insert(compact("controller", "name", "hasImage","has_page_client","main_field","lang_effected","table_name", "icon", "path", "created_at", "id"));
            ...
            $id = Request::get('id');
            //updated by fesal
            DB::table($this->table)->where('id', $id)->update(compact("name", "table_name","hasImage","has_page_client","main_field","lang_effected", "icon", "path"));
    ...
    public function postStep3()
    {
        ...
        foreach ($column as $col) {
            ...
            //added by fesal The condition was added in order not to repeat the active
            if($col=='active' || $col=='Active'){
                $i++;
                continue;
            }
        ...
        //fesal added this for alwayes show filed active in table
        $script_cols[]="\t\t\t".'$this->col[] = ["label"=>"active","name"=>"active"];';
    ...
    public function getStep3($id)
    {
        ...
        $types = [];
        foreach (glob(base_path('vendor/crud_booster_voila/crudbooster/src/views/default/type_components').'/*', GLOB_ONLYDIR) as $dir) {
            $types[] = basename($dir);
        }
    ...
    public function getTypeInfo($type = 'text')
    {
        ...
        echo file_get_contents(base_path('vendor/crud_booster_voila/crudbooster/src/views/default/type_components/'.$type.'/info.json'));
    }
