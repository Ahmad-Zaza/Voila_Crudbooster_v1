...
class MenusController extends CBController
{
    ...
    public function getIndex()
    {
        ...
        $menu_active = DB::table('cms_menus')->where('parent_id', 0)->where('is_active', 1)->where('is_front', null)->orderby('sorting', 'asc')->get();

        foreach ($menu_active as &$menu) {
            $child = DB::table('cms_menus')->where('is_active', 1)->where('parent_id', $menu->id)->where('is_front', null)->orderby('sorting', 'asc')->get();
            if (count($child)) {
                $menu->children = $child;
            }
        }

        $menu_inactive = DB::table('cms_menus')->where('parent_id', 0)->where('is_active', 0)->where('is_front', null)->orderby('sorting', 'asc')->get();
